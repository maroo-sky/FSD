...
from model.bert_kd import BertForSequenceClassification
...
STUDENT_MODEL_CLASSES = {
        "bert" : (StudentBertConfig, BertForSequenceClassification, BertTokenizer ),
}
...
def train(args, train_dataset, teacher, student, tokenizer):
    ...
    tr_cka_loss, logging_cka_loss = 0.0, 0.0
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # for external 2D CKA
                ...
                teacher_featuremaps[0] = teacher_outputs[2][12].view(args.per_gpu_train_batch_size, -1)
                student_featuremaps[0] = student_outputs[2][6].view(args.per_gpu_train_batch_size, -1)

            teacher_logit, student_logit = teacher_outputs[1], student_outputs[1]
            #estimate loss
            logit_loss = torch.nn.KLDivLoss(reduction='batchmean')
            length = len(teacher_featuremaps)

            cka_loss = cka(teacher_featuremaps, student_featuremaps, args, length)
            loss = student_outputs[0]  # model outputs are always tuple in transformers (see doc)
            dark_knowledge = logit_loss(F.log_softmax(student_logit.unsqueeze(1) / args.temperature, dim=-1),
                                        F.softmax(teacher_logit.unsqueeze(1) / args.temperature, dim=-1)) * args.temperature * args.temperature

            total_loss = args.alpha * loss + (1 - args.alpha) * dark_knowledge + args.beta * cka_loss
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    tb_writer.add_scalar("0.cross_entrophy", (tr_loss - logging_loss) / args.logging_steps, global_step)
                    tb_writer.add_scalar("1.lr", scheduler.get_lr()[0], global_step)
                    tb_writer.add_scalar("2.total_loss", (tr_total_loss - logging_total_loss) / args.logging_steps,
                                         global_step)
                    tb_writer.add_scalar("3.logit_loss", (tr_logit_loss - logging_logit_loss) / args.logging_steps,
                                         global_step)
                    tb_writer.add_scalar("cka_loss", (tr_cka_loss - logging_cka_loss) / args.logging_steps, global_step)

                    logging_loss = tr_loss
                    logging_total_loss = tr_total_loss
                    logging_logit_loss = tr_logit_loss
                    logging_cka_loss = tr_cka_loss
...
def main():
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, teacher_model, student_model, tokenizer)
